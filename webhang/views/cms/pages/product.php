  ...
  <h1 class="h3 mb-2 text-gray-800 d-sm-flex align-items-center justify-content-between mb-4">
    ...
    <div class="modal fade" id="addProduct" tabindex="-1" role="dialog" aria-labelledby="addProductModal" aria-hidden="true">
      <div class="modal-dialog" role="document">
        <form method="POST" action="http://localhost:90/projectsphp/webbanhang/webhang/controller/cms/product.php" enctype="multipart/form-data">
          ...
          <tbody>
            <?php foreach ($list as $value) { ?>
              <tr>
                ...
                <td>
                  <button class="btn btn-danger" onclick="confirm('aaa')?deleteProduct(<?= $value->id ?>):alert('xxx')" data-id="<?= $value->id ?>">Delete</button>
                  <button class="btn btn-warning" onclick="editProduct(<?= $value->id ?>)" data-id="<?= $value->id ?>">Edit</button>
                </td>
              </tr>
            <?php } ?>
          </tbody>
...
<script>
  function deleteProduct(id) {
    $.post("http://localhost:90/projectsphp/webbanhang/webhang/controller/cms/product.php", {
      id: id,
      action: "delete"
    }, function(data) {
      // console.log(data);
